...
def parse(pathobj):
    out = ""
    lastcommand = None
    turn_off_plasma_z = None
    turn_on_plasma_z = None
    feedrate_set = False
    precision_string = '.' + str(PRECISION) + 'f'
    global SUPPRESS_TOOL_CHANGE

    # params = ['X','Y','Z','A','B','I','J','K','F','S'] #This list control the order of parameters
    params = ['X', 'Y', 'Z', 'A', 'B', 'I', 'J', 'S', 'T', 'Q', 'R',  # Got rid of F
              'L']  # linuxcnc doesn't want K properties on XY plane  Arcs need work.
    ...
    if hasattr(pathobj, "Group"):  # We have a compound or project.
        ...
        for c in pathobj.Path.Commands:
            ...
            if command == 'G0' and 'Z' in c.Parameters:
                if turn_off_plasma_z is None:
                    turn_off_plasma_z = int(c.Parameters['Z'])
                    turn_on_plasma_z = turn_off_plasma_z - 2

            if command == 'G1' and feedrate_set is False:
                outstring.append('S1000 F1000')
                # c.Parameters['F'] = 1000
                # c.Parameters['S'] = 1000
                feedrate_set = True
                #outstring.append("; FOUND FIRST G1")
            ...
            if command == 'G0' and 'Z' in c.Parameters:
                if len(c.Parameters) == 1:
                    if c.Parameters['Z'] == float(turn_off_plasma_z):
                        outstring.clear()
                        outstring.append(FLOATING_Z_COMMANDS_TURN_OFF)
                    elif c.Parameters['Z'] == float(turn_on_plasma_z):
                        outstring.clear()
                        outstring.append(FLOATING_Z_COMMANDS_TURN_ON)
                if len(c.Parameters) >= 2:
                    if c.Parameters['Z'] == float(turn_on_plasma_z):
                        outstring.clear()
                        outstring.append(FLOATING_Z_COMMANDS_TURN_ON)
